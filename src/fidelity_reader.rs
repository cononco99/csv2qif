...
#[derive(Clone, Copy)]
pub struct FidelityReader;

impl CsvReader for FidelityReader {
    fn csv_header(&self) -> String {
        r#"Run Date,Action,Symbol,Security Description,Security Type,Quantity,Price ($),Commission ($),Fees ($),Accrued Interest ($),Amount ($),Settlement Date"#
            .to_string()
    }
    ...
    fn to_transactions(
        ...
        let fidelity_transactions_reversed: Vec<FidelityTransaction> =
            fidelity_transactions.iter().rev().cloned().collect(); // we want oldest first
        let mut symbols = Symbols::new(current_securities_file)?;

        let from_fidelity_transaction = |tr| FidelityTransaction::to_qif_action(tr, &mut symbols);
        let nested_actions = fidelity_transactions_reversed
            .iter()
            .map(from_fidelity_transaction)
...
impl FidelityReader {
    fn read_transactions_csv(bufreader: &mut dyn BufRead) -> Result<Vec<FidelityTransaction>> {
        ...
        for result in rdr.deserialize() {
            ...
            if let Ok(record) = result {
                ...
                let mut cleaned_record: FidelityTransaction = record;
                ...
                // fidelity has one bad line at end of csv file.
                should_be_done = true;
            }
        }
        Ok(transactions)
...
pub struct FidelityTransaction {
...
impl FidelityTransaction {
    ...
    fn security_details(&self) -> Result<(String, String, SecurityType)> {
        let option_result = FidelityTransaction::get_option(self);
        match option_result {
            ...
            Err(_) => {
                ...
                if let Some(cap) = second_try_re.captures_iter(&self.date).next() {
                    ...
                    match second_try {
                        ...
                        Err(_) => {
                            let err_msg = "Could not parse date from fidelity on second try: "
                                .to_string()
                                + &self.date;
                ...
                let err_msg = "Could not match date from fidelity: ".to_string() + &self.date;
                Err(eyre!(err_msg))
            }
        }
    }
    ...
    fn to_transaction(
        fidelity_transaction: &FidelityTransaction,
        symbols: &mut Symbols,
    ) -> Result<Transaction> {
        let (symbol, name, security_type) = fidelity_transaction.security_details()?;

        let price = fidelity_transaction.price.to_string();
        let quantity = fidelity_transaction.quantity.to_string()
            + if security_type == SecurityType::Option {
                "00"
            } else {
                ""
            };
        let amount = fidelity_transaction
            .amount
            .trim_start_matches('-')
            .to_string();
        let fees = fidelity_transaction.fees.to_string();
        let date: NaiveDate = fidelity_transaction.get_date()?;
    ...
    fn to_expired_transaction(
        ...
        // negation hack
        let (leading, rest) = fidelity_transaction.quantity.split_at(1);
        let q = if leading == "-" {
            rest.to_string()
        } else {
            "-".to_string() + &fidelity_transaction.quantity
        };
    ...
    fn to_qif_action(
        ...
        let csv_action = fidelity_transaction.action.as_str();
        match csv_action {
            "Sell to Open" => {
                let transaction = Self::to_transaction(fidelity_transaction, symbols)?;
            ...
            "Expired" => {
                let transaction = Self::to_expired_transaction(fidelity_transaction, symbols)?;
                res.push(QifAction::SellX { transaction });
            }
            "Margin Interest" => {
                // Margin Interest from fidelity is negative but quicken wants it positive.
                // Hence the trim_start_matches hack for amount
                res.push(QifAction::MargIntX {
                    date: fidelity_transaction.get_date()?,
                    memo: fidelity_transaction.description.clone(),
                    amount: fidelity_transaction
                        .amount
                        .trim_start_matches('-')
                        .to_string(),
                });
            ...
            "Cash Dividend" => {
                ...
                res.push(QifAction::DivX {
                    date: fidelity_transaction.get_date()?,
                    symbol,
                    amount: fidelity_transaction.amount.clone(),
                });
            }
            "Qualified Dividend" => {
                let (symbol, name, security_type) = fidelity_transaction.security_details()?;
            ...
            | "MoneyLink Transfer" | "Pr Yr Cash Div" | "Pr Yr Cash Div Adj" | "Bank Interest" => {
                res.push(QifAction::LinkedAccountOnly {
                    date: fidelity_transaction.get_date()?,
                    payee: fidelity_transaction.description.clone(),
                    memo: fidelity_transaction.description.clone(),
                    amount: fidelity_transaction.amount.clone(),
                });
            }
            ...
            "Spin-off" => {
                let (symbol, name, security_type) = fidelity_transaction.security_details()?;
                let quantity = fidelity_transaction.quantity.parse::<i32>()?;
                let date: NaiveDate = fidelity_transaction.get_date()?;
                symbols.enter_if_not_found(&symbol, &name, &security_type)?;
            ...
            "Stock Split" => {
                ...
                println!("This is because Fidelity CSV contains the number of new shared added due to the split but quicken records the factor that the old number of shared is multiplied by to get the new number of shares.  Without knowing the starting number of shares, the factor can not be calculated.  The split will have to be entered by hand:");
                println!("{:#?}", fidelity_transaction);
                println!();
            }
            ...
            _ => {
                if (fidelity_transaction.quantity.is_empty())
                    && (fidelity_transaction.price.is_empty())
                    && (fidelity_transaction.fees.is_empty())
                {
